...
import { Send, Save, X, Check, Terminal, Pencil } from 'lucide-react';
...
import CodeSnippet from './request/CodeSnippet';
...
export default function RequestEditor() {
    ...
    const [currentTab, setCurrentTab] = useState<'params' | 'headers' | 'body' | 'auth' | 'scripts' | 'code'>('params');
    ...
    const buildCurl = () => {
        ...
        if (activeRequest.body?.type === 'json' && activeRequest.body.content) {
            const content = typeof activeRequest.body.content === 'string'
                ? activeRequest.body.content
                : JSON.stringify(activeRequest.body.content);
            bodyFlag = `-H "Content-Type: application/json" -d '${content.replace(/'/g, `'\\''`)}'`;
        } else if (activeRequest.body?.type === 'raw' && activeRequest.body.content) {
            bodyFlag = `-d '${String(activeRequest.body.content).replace(/'/g, `'\\''`)}'`;
        }
        const authFlag = (() => {
            const a = activeRequest.auth;
            if (!a || a.type === 'none') return '';
            if (a.type === 'bearer') return `-H "Authorization: Bearer ${a.bearer?.token || ''}"`;
            if (a.type === 'basic') return `-u "${a.basic?.username || ''}:${a.basic?.password || ''}"`;
            if (a.type === 'apikey' && a.apikey?.addTo === 'header') return `-H "${a.apikey.key}: ${a.apikey.value}"`;
            return '';
        })();
        return `curl -X ${activeRequest.method} "${activeRequest.url}" ${headers} ${authFlag} ${bodyFlag}`.replace(/\s+/g, ' ').trim();
    };
        ...
        <div className="flex flex-1 flex-col overflow-hidden bg-gray-900">
            ...
            <div className="border-b border-gray-800 px-6 py-1.5 flex items-center justify-between shrink-0">
                ...
                <button
                    ...
                    title="Copy as cURL"
                >
                    {curlCopied ? <Check className="h-3.5 w-3.5 text-green-400" /> : <Terminal className="h-3.5 w-3.5" />}
            ...
            <div className="border-b border-gray-800 shrink-0">
                <div className="flex gap-1 px-6">
                    {(['params', 'headers', 'body', 'auth', 'scripts', 'code'] as const).map((tab) => {
            ...
            <div ref={containerRef} className="flex flex-1 flex-col overflow-hidden">
                <div style={{ height: requestPanelHeight }} className="overflow-auto p-4 shrink-0">
                    ...
                    {currentTab === 'code' && <CodeSnippet request={activeRequest} />}
                </div>
